...
export default function CoolDownPage() {
  ...
  const cooldown = weekData?.cooldown
  if (!cooldown) return (
    <div className="min-h-screen bg-[#007A3D] flex items-center justify-center">
      <p className="text-white">Cool down not set up for this week.</p>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#007A3D] px-4 pt-safe pb-8">
      <div className="flex items-center gap-4 pt-5 pb-4">
        ...
        <div>
          <p className="text-white/60 text-xs">Cool Down</p>
      ...
      {cooldown.image && (
        <div className="rounded-2xl overflow-hidden mb-5 bg-white/10">
          <img src={cooldown.image} alt={cooldown.name} className="w-full object-contain max-h-48" />
        </div>
      )}
